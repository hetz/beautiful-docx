...
class Image {
    constructor(imageFigure, parentTag, exportOptions) {
    ...
    createOptions(imageBuffer, imageSourceUrl) {
    ...
    getImageType(imageBuffer, imageSourceUrl) {
    ...
    transformToDocx() {
        if (['td', 'li'].includes(this.parentTag)) {
            return [new docx_1.Paragraph({ children: [new docx_1.ImageRun(this.options)] })];
        }
        else {
