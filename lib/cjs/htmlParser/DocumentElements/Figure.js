...
class Figure {
    constructor(element, docxExportOptions) {
        ...
        if (tableNode) {
            this.content = new Table_1.TableCreator(tableNode, 'table', docxExportOptions).getContent();
        }
        else if (imageNode) {
            this.content = new Image_1.Image(element, 'img', docxExportOptions).getContent();
        }
        else {
