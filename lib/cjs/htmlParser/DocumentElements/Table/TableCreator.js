...
class TableCreator {
    constructor(element, parentTag, exportOptions) {
        ...
        this.options = {
            layout: docx_1.TableLayoutType.AUTOFIT,
        ...
        for (const tableChild of this.element.children) {
            ...
            switch (tableChild.tagName) {
                ...
                case 'tfoot':
                    ...
                    break;
                case 'tr':
                    this.children.push(...new TableRow_1.TableRow(tableChild, false, this.exportOptions).getContent());
                    break;
                case 'colgroup':
    ...
    parseTableRowsFragment(element, isHeader) {
        ...
        for (const child of element.children) {
            ...
            switch (child.tagName) {
                case 'tr':
                    rows.push(...new TableRow_1.TableRow(child, isHeader, this.exportOptions).getContent());
                    break;
                default:
    ...
    parseCaption(element) {
        return new HtmlParser_1.HtmlParser(this.exportOptions).parseHtmlTree(element.children, 'caption').map((i) => {
